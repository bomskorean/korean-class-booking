...
async function main() {
  ...
  console.log("Seeding ticket packages...");
  for (const pkg of PACKAGES) {
    ...
    console.log(`  ✓ Package: ${pkg.name} — ¥${pkg.price.toLocaleString()} (¥${pkg.unitPrice}/回, ${pkg.validMonths}か月)`);
  }
  ...
  const devStudents = [
    ...
    { name: "鈴木 一郎",                email: "ichiro@dev.local", pkgId: "pkg_8",  remaining: 2,
      validFrom: new Date(Date.now() - 30 * 86_400_000) },   // 1ヶ月前に開始
    ...
    if (s.pkgId) {
      ...
      if (pkg) {
        ...
        await prisma.userTicket.upsert({
          where:  { id: ticketId },
          update: { remainingCount: s.remaining },
          create: {
            id: ticketId,
            userId: user.id,
            packageId: s.pkgId,
